...
@Component({
  ...
  templateUrl: './form-pago-total.component.html',

})
export class FormPagoTotalComponent implements OnInit {
  pagosInfo!: FormGroup;
  public tours: any = [];
  ...
  createpagosInfo(){
    ...
    if(this.descuentoElegido!=undefined){
      if(this.descuentoElegido!=0){
        this.totalDescuento =
        this.totalCompra - this.totalCompra*(
          (this.descuentos[this.descuentoElegido].porcentaje)/100
          )
        this.totalDescontado =this.totalCompra*((this.descuentos[this.descuentoElegido].porcentaje)/100);
      }else{
        this.totalDescuento =
        this.totalCompra-this.tourSeleccionado.paquete.precio*((this.descuentos[this.descuentoElegido].porcentaje)/100)
    ...
    if(this.descuentoElegido==undefined){
      this.totalDescuento = this.totalCompra;
    }
